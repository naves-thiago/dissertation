...
function love.update(dt)
	...
	if show_login then
		...
		if suit.Button(login_button_text, {id = 1}, login_left, login_top + 150,
			300, 30).hit then
			check_login(login_user.text, login_pass.text)
		end

		suit.Label(login_status_text, {align = 'left'}, login_left, login_top + 220,
			300, 10)
	end
end
